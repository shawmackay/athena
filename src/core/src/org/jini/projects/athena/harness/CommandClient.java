...
/**
 * Client program that accepts the name of a Command Harness class and executes
 * it. <br/><br/>Usage: <br/>
 * 
 * <pre>
 * 
 *   java [jvm-options] org.jini.projects.athena.harness.Client [harnessname] [ABORT|COMMIT] [L|D|R] [ATHENANAME] [JINIGROUP] [number of runs (default 1)] [keepconnection (Y|N) (default N)]
 *  
 * </pre>
 * 
 * Explanation of options: <br/><table border="1">
 * <tr>
 * <td width=100><b><center>harnessname </center> </b></td>
 * <td>name of the class to execute in the command</td>
 * </tr>
 * <tr>
 * <td><b><center>Transaction <br/>Completion </center> </b></td>
 * <td>Whether you wish to either COMMIT or ABORT all of this command set</td>
 * </tr>
 * <tr>
 * <td><b><center>Resultset <br/>Type </center> </b></td>
 * <td>Whether you wish to use Local, Disconnected or Remote ResultSets</td>
 * </tr>
 * <tr>
 * <td><b><center>AthenaName <br/>& <br/>Jini Group </center> </b></td>
 * <td>Which group and name the client will use to find the Athena instance you
 * require</td>
 * </tr>
 * <tr>
 * <td><b><center>Number <br/>of Runs </center> </b></td>
 * <td>How many loops do you want the client to perform(all operations in a
 * single XML file on a single loop are executed under a single transaction)
 * </td>
 * </tr>
 * <tr>
 * <td><b><center>Keep Connection </center> </b></td>
 * <td>Y/N - Keep the same connection between runs, or return it and get a new
 * connection on each run</td>
 * </tr>
 * </table> <br/>
 * 
 * Commands run through this harness should have parameters that are all
 * strings. Support to define other object types will be added in a future
 * version.
 * 
 * @author calum
 *  
...
public class CommandClient implements DiscoveryListener {
	...
	public static void main(String[] args) {
		...
		CommandClient ptctClient1 = new CommandClient();
	}
		...
		try {
			if (!discovered) {
				...
				if (athReg != null) {
					...
					System.out.println(" No of runs: " + NUMRUNS);
					for (int j = 0; j < NUMRUNS; j++) {
						if (!keepconnection) {
							conn = athReg.getConnection("ClientHarness", 2 * TimeConstants.MINUTES);
							 System.out.println("Creating new connection for run");
						}
						if (conn == null) {
	...
	private void doTransactedWork(AthenaConnection connection, int local, int i) {
		if (connection != null && this.txmgr != null) {
			...
			try {
				...
				Command comm = connection.getCommand();
				CommandHarness ch = (CommandHarness) Class.forName(HARNESS).newInstance();
				ch.populateCommand(comm);
				if (ch instanceof ReaderHarness) {
					ReaderHarness theHarness = (ReaderHarness) ch;
					AthenaResultSet rs = connection.executeQuery(comm);
					theHarness.handleResultSet(rs);
				}if (ch instanceof ObjectHarness) {
					ObjectHarness theHarness = (ObjectHarness) ch;
					Object o= connection.executeObjectQuery(comm);
					System.out.println("O is: " + o);
					theHarness.handleReturnObject(o);
				} else
					connection.executeUpdate(comm, tx);
	...
	// public void loadXml(String filename, AthenaConnection conn, Transaction
	// tx) {
	// //System.out.println("Loading " + filename);
	// try {
	// File f = new File(filename);
	// try {
	// BufferedInputStream b_xmlin = new BufferedInputStream(new
	// FileInputStream(f));
